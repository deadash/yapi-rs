...
namespace yapi {
	...
	typedef std::basic_string<wchar_t, std::char_traits<wchar_t>, std::allocator<wchar_t> > tstring;
		...
		static std::string _W2T(const wchar_t* wcs)
		{
			...
			assert(0 != ::WideCharToMultiByte(CP_ACP, 0, wcs, -1, &ret[0], len, 0, 0));
			ret.resize(len - 1);
			return ret;
		}
	#else
	...
	DWORD64 WINAPI GetModuleHandle(HANDLE hProcess, const wchar_t* moduleName)
	{
		...
		if (Module32FirstW(hSnap, &mod)) {
			do {
				if (!_wcsicmp(mod.szModule, moduleName)) {
					CloseHandle(hSnap);
	...
	DWORD64 WINAPI GetModuleHandle64(HANDLE hProcess, const wchar_t* moduleName)
	{
		if (!moduleName) return 0;
		if (!hProcess) hProcess = detail::hCurProcess;
		...
		do {
			...
			if (!_wcsicmp(moduleName, modName.c_str()))
				return head.DllBase;
	...
	DWORD64 GetNtDll64()
    {
		...
		hNtdll64 = GetModuleHandle64(detail::hCurProcess, L"ntdll.dll");
		return hNtdll64;
	}
	...
	public:
		...
		YAPICall(HANDLE hProcess, const wchar_t* modName, const char* funcName)
			: _hProcess(hProcess)
			, _sc(0)
